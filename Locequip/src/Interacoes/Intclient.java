...
//Prepara a conexão p enviar o código sql para o banco de dados.
    String sql = "Insert into cadastroc (nomeC, rgC, cpfC, emailC, senhaC, telefoneC)" 
            + "values (?, ?, ?, ?, ?, ?)";
...
stmt.setInt (2,v1.getrgC());
stmt.setInt (3,v1.getcpfC());
stmt.setString (4,v1.getemailC());
stmt.setString (5,v1.getsenhaC());
stmt.setInt (6,v1.gettelefoneC());
...
public List <VariaveisC> getLista (String f1) throws SQLException {
    ...
    String sql= "select * from cadastroc where nomeC like ?";
 ...
 while (rs.next()){
     ...
     v1.setrgC (rs.getInt ("rgC"));
     v1.setcpfC (rs.getInt ("cpfC"));
     v1.setemailC(rs.getString ("emailC"));
     v1.setsenhaC(rs.getString ("senhaC"));
     v1.settelefoneC(rs.getInt ("telefoneC"));
...
public List <VariaveisC> getLista () throws SQLException {
 String sql= "select * from cadastroc";
...
String sql = "update cadastroc set nomeC = ?, rgC = ?, cpfC = ?, emailC = ?, senhaC = ?, telefoneC = ?"+" where idCliente = ?";
...
String sql = "delete from cadastroc where idCliente=?";
PreparedStatement stmt = conexao.prepareStatement(sql);
